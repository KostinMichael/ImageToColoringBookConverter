...
namespace ImageQuantizer
{
    class ImageQuantizer
    {
        ...
        {
            List<Pixel[]> paletteFull = new List<Pixel[]>();
            paletteFull.Add(pixels);
            Pixel[] buf, buf2;
            int k = 0, indexOfColor;
            while (paletteFull.Count - k < paletteSize)   //хуета
            {
                if (paletteFull[k].Length > 1)
                {
                    pixels = paletteFull[k];
                    ...
                    paletteFull.Add(buf);
                    paletteFull.Add(buf2);
                    paletteFull.RemoveAt(k);
            ...
            for (int i = 0; i < paletteFull.Count; i++)
            {
                if (paletteFull[i].Length > 1)
                {
                    sumR = 0; sumG = 0; sumB = 0;
                    for (int j = 0; j < paletteFull[i].Length; j++)
                    {
                        sumR += paletteFull[i][j].R;
                        sumG += paletteFull[i][j].G;
                        sumB += paletteFull[i][j].B;
                    }
                    palette[0, i] = (byte)(sumR / paletteFull[i].Length);
                    palette[1, i] = (byte)(sumG / paletteFull[i].Length);
                    palette[2, i] = (byte)(sumB / paletteFull[i].Length);
                }
                else
                {
                    palette[0, i] = paletteFull[i][0].R;
                    palette[1, i] = paletteFull[i][0].G;
                    palette[2, i] = paletteFull[i][0].B;
                }
            }
            return palette;
        }
        ...
        public Bitmap GetMedianSmoothImage(int radius)
        {
            Pixel[,] pixels = convertArrayToPix(qByteIMG);
            List<Pixel> medianArea;
            byte[,,] newQByteIMG = new byte[qByteIMG.GetLength(0), qByteIMG.GetLength(1), qByteIMG.GetLength(2)];
            for (int posH = radius; posH < qByteIMG.GetLength(1) - radius; posH++)
            {
                for (int posW = radius; posW < qByteIMG.GetLength(2) - radius; posW++)
                {
                    medianArea = new List<Pixel>();
                    for (int h = posH - radius; h <= posH + radius; h++)
                    {
                        for (int w = posW - radius; w <= posW + radius; w++)
                        {
                            if (!(w == posW && h == posH ))
                            {
                                int t = medianArea.IndexOf(pixels[h, w]);
                                if (t != -1)
                                {
                                    medianArea[t].Size++;
                                }
                                ...
                                {
                                    medianArea.Add(new Pixel(pixels[h, w].R, pixels[h, w].G, pixels[h, w].B));
                                }
                            }
                        }
                    }
                    medianArea.Sort(new SizeComparer());
                    Pixel main = medianArea[medianArea.IndexOf(pixels[posH, posW])];
                    if (!pixels[posH, posW].Equals(medianArea[medianArea.Count-1]) && medianArea[medianArea.Count-1].Size > main.Size)
                    {
                        newQByteIMG[0, posH, posW] = medianArea[medianArea.Count - 1].R;
                        newQByteIMG[1, posH, posW] = medianArea[medianArea.Count - 1].G;
                        newQByteIMG[2, posH, posW] = medianArea[medianArea.Count - 1].B;
                    }
                    else {
                        newQByteIMG[0, posH, posW] = pixels[posH, posW].R;
                        newQByteIMG[1, posH, posW] = pixels[posH, posW].G;
                        newQByteIMG[2, posH, posW] = pixels[posH, posW].B;
                    }
                }
            }
            qByteIMG = newQByteIMG;
            return bitmapConverter.rgbToBitmapQ(newQByteIMG);
        }
    }
    ...
    public class SizeComparer : IComparer<Pixel>
    {
        int IComparer<Pixel>.Compare(Pixel x, Pixel y)
        {
            return ((Pixel)(x)).Size - ((Pixel)(y)).Size;
        }
    }
}
